...
<form class="form-horizontal" method="post"
      action="<?= BASE_URL ?>/activiteAdmin/deplacerCreneau/<?= $donnees->ID ?>">
...
<form method="POST" action="<?= BASE_URL . '/activiteAdmin/paye/' . $donnees->ID ?>">


    <?php if ($donnees->PAYE == 1) {
        echo 'Payé le ' . date_format(date_create($donnees->DATE_PAIEMENT), 'd-m-Y');
    } elseif ($donnees->PAYE == 0) {
        ?>
        <button class="btn btn-success">Valider le paiement</button> <?php
    } ?>
...
<form method="POST" action="<?= BASE_URL . '/activiteAdmin/desinscrire/' . $donnees->ID ?>">
